...
:: ======================================================================================================================= ::
    ...
    if exist "%recoveryDrive%\ComponentPackageInstallation" (
        echo] > "%systemdrive%\ComponentPackageInstallation"
        del /f /q "%recoveryDrive%\ComponentPackageInstallation"
    ) else (
        echo] > "%systemdrive%\NormalWindowsRecovery"
    ...
    set "packagesList=%targetDrive%\Windows\EudynOS\winrePackagesToInstall"
    set "LogDirectory=%targetDrive%\Windows\EudynOS\Logs"

    :: Other variables
    set "percentage=1"
    set "dontRestartPath=%systemdrive%\DontRestart"

    :: Set log file
    echo INFO: Setting log file...
    if not exist "%LogDirectory%" mkdir "%LogDirectory%" & echo INFO: Made log directory...
    ...
    set /a logNum += 1
    set "logDirPath=%LogDirectory%\%logNum%-WinRE-PackageInstall-%formattedDate%"
    ...
    start /i /d "%targetDrive%\Windows" /min "Debug Console" cmd /k %currentDirectory%debug.cmd "%logPath%" "%dontRestartPath%" "%focusPath%"
...
:: -------------------------------------------------------------------------------------------------------- ::
    ...
    if defined error echo "%logPath%" "%error%" > "%targetDrive%\Windows\System32\PackagesFailure"
...
:: ======================================================================================================================= ::
    ...
    del /f /q "%recoveryDrive%\bitlocker.txt" > nul 2>&1
    exit /b
