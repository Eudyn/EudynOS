...
function SafeMode {
    ...
    if ($Enable) {
        ...
        $shellValue = "powershell -NoP -EP Unrestricted -File `"$envPath\main.ps1`" $arguments"
    } else {
...
# Create log folder
$LogDirectory = "$modules\Logs"
if (!(Test-Path $LogDirectory)) { New-Item -Path $LogDirectory -Force -ItemType Directory | Out-Null }
do {
    $sessionLogDirectory = "$LogDirectory\$($num++; $num)-OnlinePackageInstall-$((Get-Date).ToString('dd-MM-yyyy'))"
...
$useWinreIndicator = "$envPath\UseWindowsRecovery"
...
# ======================================================================================================================= #
if (!$safeModeWithList) {
    $packageList = (Get-ChildItem "$env:windir\EudynOS\Packages\*.cab").FullName | Where-Object {$_ -like "*$(if ($arm) {'arm64'} else {'amd64'})*"}
...
if ($failedPackages.Count -ne 0) {
    if (!$safeMode) {
        ...
        $safeModeStartupTitle = 'SafeModeComponentCheck'
        ...
        $arguments = "/c title 'Finalizing installation ' ; echo 'Do not close this window and not turn off your computer.' ; echo 'Your computer will automatically restart.' ; schtasks /delete /tn `"$safeModeStartupTitle`" /f > nul"
        + "powershell -NoP -EP Unrestricted -C `"& '$envPath\main.ps1' -WinRE`" & pause"
