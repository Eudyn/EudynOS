# Made by Atlas Team and adjusted by Eudyn Team for EudynOS.
...
$requiredPaths = @{
    ...
    modules = "$env:windir\EudynOS"
    EnvironmentItems = "$env:windir\EudynOS\PackagesEnvironment\recovery"
    WinrePackages = "$env:windir\EudynOS\winrePackagesToInstall"
...
$bitlockerTaskName = 'BitlockerRemovalTask'
...
# Misc
$failurePath = "$env:windir\System32\EudynOSPackagesFailure"
# https://ss64.com/vb/popup.html
$sh = New-Object -ComObject "Wscript.Shell"

# Messages
$TeamOnGit = 'Report this to the EudynOS team.'
...
function FatalError ($recoveryError) {
    $null = $sh.Popup($recoveryError,20,'Components Error',0+48)
}

function NoDiskSpaceError ($space) {
    FatalError @"
You need more than $space GB of free disk space' component removals to work.

Without this, you won't be able to use disable Defender, remove telemetry and have EudynOS do other removals.
...
function StartupTask {
    $arguments = '/c title Finalizing installation & echo Do not close this window and not turn off your computer. & schtasks /delete /tn "RecoveryFailureCheck" /f > nul & ' `
        + 'powershell -NoP -EP Unrestricted -WindowStyle Hidden -C "& $(Join-Path $env:windir ''\EudynOS\PackagesEnvironment\winrePackages.ps1'') -NextStartup"'
...
if ($NextStartup -and (Test-Path $failurePath)) {
    $WindowTitle = 'Failed removing components'
    ...
    $PackagesFailure = Get-Content $failurePath
    $logPath = ($PackagesFailure -split '"')[1] -replace [regex]::Escape($env:windir), '%windir%'
    $errorLevel = ($PackagesFailure -split '"')[3]
...
"@; $DevicePath = [System.Text.StringBuilder]::new(255)
        ...
        do {
            $mountPoint = Join-Path -Path $env:WinDir -ChildPath ("Recovery." + $([System.IO.Path]::GetRandomFileName()))
    ...
    $bitlockerRecoveryKeyTxt = "$mountPoint\bitlocker.txt"
    $componentInstallationIndicator = "$deviceDrive\ComponentPackageInstallation"
    ...
    if ($bitlockerVolume.ProtectionStatus -eq 'On') {
        ...
        if ($recoveryPartition) {
            ...
            $action = New-ScheduledTaskAction -Execute 'cmd' `
                    -Argument '/c schtasks /delete /tn "BitlockerRemovalTask" /f > nul & powershell -EP Unrestricted -WindowStyle Hidden -NoP & $(Join-Path $env:windir ''\EudynOS\PackagesEnvironment\winrePackages.ps1'') -DeleteBitLockerPassword'
...
"@,0,$WindowTitle,0+16)
    ...
    do {
        $WinreWim = Join-Path -Path $env:WinDir -ChildPath ("WinreWim." + $([System.IO.Path]::GetRandomFileName()))
    } until (!(Test-Path $WinreWim))
    New-Item -Path $WinreWim -Type Directory -Force | Out-Null
    Mount-WindowsImage -ImagePath $fullWimPath -Index 1 -Path $WinreWim | Out-Null
    if (!$?) {
...
"@,0,$WindowTitle,0+16)
    ...
    [IO.File]::WriteAllLines("$WinreWim\Windows\System32\winpeshl.ini", @"
[LaunchApps]
%WINDIR%\System32\wscript.exe, %SYSTEMDRIVE%\EudynOS\startup.vbs //B
"@)

    Write-Info -Text 'Copying files...'
    Copy-Item "$EnvironmentItems\*" -Destination "$WinreWim\EudynOS" -Recurse -Force    
    ...
    if ((Test-Path "$WinreWim\Windows") -and $WinreWim) {
        Dismount-WindowsImage -Path $WinreWim -Save | Out-Null
        if (!$?) {
